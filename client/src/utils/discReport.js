...
export function generateDISCReport(result, userName = '') {
  ...
  // Strip characters outside Latin-1 range (jsPDF Helvetica can't render them)
  function safe(str) {
    return String(str).replace(/[^\x00-\xFF]/g, '').replace(/—/g, '-').replace(/–/g, '-');
  }
  ...
  y += 22;
  text(safe(PROFILES[primary].tagline), PW / 2, 10, [255, 255, 255], false, null, 'center');
  ...
  sorted.forEach(([type, score]) => {
    ...
    pdf.text(safe(`${type} - ${PROFILES[type].name}`), MARGIN + 10, y + 18);
  ...
  pProfile.strengths.forEach(s => {
    ...
    const lines = pdf.splitTextToSize(safe(s), CW - 20);
  ...
  pProfile.blindspots.forEach(b => {
    ...
    pdf.circle(MARGIN + 8, y - 3, 2, 'F');
    const lines = pdf.splitTextToSize(safe(b), CW - 20);
    pdf.text(lines, MARGIN + 16, y);
    y += lines.length * 13;
  });
  ...
  pProfile.develop.forEach((d, i) => {
    ...
    const lines = pdf.splitTextToSize(safe(d), CW - 30);
  ...
  months.forEach((month, mi) => {
    ...
    targetStyles.forEach(style => {
      ...
      y += 13;
      pdf.setFontSize(9); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(...COLORS.text);
      const lines = pdf.splitTextToSize(safe(action), CW - 18);
  ...
  y += 18;
  text('Quarterly Self-Check (every 90 days)', MARGIN + 14, 10, COLORS.navy, true);
  y += 14;
  ...
  Object.entries(COMMUNICATION_TIPS).forEach(([type, info]) => {
    ...
    info.tips.forEach(tip => {
      ...
      pdf.setFontSize(9); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(...COLORS.text);
      pdf.circle(MARGIN + 8, y - 3, 2, 'F');
      const lines = pdf.splitTextToSize(safe(tip), CW - 20);
      pdf.text(lines, MARGIN + 16, y);
      y += lines.length * 13;
    });
    y += 12;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    pdf.text(safe(`DISC Report - ${userName || 'Participant'} | Page ${i} of ${totalPages}`), PW / 2, 828, { align: 'center' });
  }

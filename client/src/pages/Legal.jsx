...
// the in-app English copy always matches the exported Word document. The
// Spanish text is a translation layer on top of that same English source of
// truth (kept untouched in legalContent.js), keyed by document + section
// index so it never drifts out of sync with the section order.
// Public pages — reachable before login and linked from the signup form and footer.
function LegalPage({ docKey, title, subtitle, sections }) {
    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9', padding: '2rem 1rem' }}>
      <div style={{ maxWidth: 820, margin: '0 auto' }}>
        <div style={{ background: 'linear-gradient(135deg, #0f2044 0%, #1e3a6e 60%, #0d9488 100%)', color: 'white', borderRadius: '16px 16px 0 0', padding: '1.75rem 2rem' }}>
          <Link to="/login" style={{ color: '#99f6e4', fontSize: '0.8rem', fontWeight: 700, textDecoration: 'none' }}>← {t('legal.backToSignIn', 'Back to sign in')}</Link>
          ...
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.78rem', margin: '8px 0 0' }}>
            {t('legal.effectiveDate', 'Effective date')}: {LEGAL_META.effectiveDate}
          </p>
        </div>
        <div style={{ background: 'white', borderRadius: '0 0 16px 16px', padding: '1.75rem 2rem', boxShadow: '0 12px 40px rgba(15,32,68,0.08)' }}>
          {sections.map((sec, si) => (
            <section key={sec.heading} style={{ marginBottom: 22 }}>
              <h2 style={{ fontSize: '1rem', fontWeight: 800, color: '#0f2044', margin: '0 0 8px' }}>{t(`legal.${docKey}.s${si}.heading`, sec.heading)}</h2>
              {sec.paragraphs.map((p, i) => (
                <p key={i} style={{ fontSize: '0.9rem', color: '#334155', lineHeight: 1.65, margin: '0 0 8px' }}>{t(`legal.${docKey}.s${si}.p${i}`, p)}</p>
              ))}
            </section>
          ))}
...
export function Terms() {
  const { t } = useTranslation();
  return <LegalPage docKey="terms" title={t('legal.termsConditions', 'Terms & Conditions')} subtitle={t('legal.termsSubtitle', '{{appName}} — please read these terms carefully.', { appName: LEGAL_META.appName })} sections={TERMS_SECTIONS} />;
}

export function Privacy() {
  const { t } = useTranslation();
  return <LegalPage docKey="privacy" title={t('legal.privacyPolicy', 'Privacy Policy')} subtitle={t('legal.privacySubtitle', 'How the {{appName}} handles your data.', { appName: LEGAL_META.appName })} sections={PRIVACY_SECTIONS} />;
}
